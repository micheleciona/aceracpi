 ...
 *
 * Disassembly of 157, Tue Jan  8 15:14:42 2008
 *
 *
 * Original Table Header:
 *     Signature        "DSDT"
 *     Length           0x000046F3 (18163)
 *     Revision         0x01
 *     OEM ID           "INTEL "
 *     OEM Table ID     "ODEM    "
 ...
 */
DefinitionBlock ("157.aml", "DSDT", 1, "INTEL ", "ODEM    ", 0x06040000)
{
    ...
    OperationRegion (MNVS, SystemMemory, 0x3FF7BE7D, 0x40)
            ...
            Device (LPCB)
            {
                ...
                Device (EC0)
                {
                    ...
                    Method (_Q0C, 0, NotSerialized)
                    {
                        ...
                        {
                            If (LEqual (\_SB.PCI0.AGP.VGA.TOGF, 0x0F))
                            {
                                Store (0x2B, SMIF)
                                Store (0x00, TRP0)
                                If (LGreaterEqual (PAR1, 0x02))
                                {
                                    Store (0x00, \_SB.PCI0.AGP.VGA.TOGF)
                                }
                                Else
                                {
                                    Store (Increment (PAR1), \_SB.PCI0.AGP.VGA.TOGF)
                                }
                            }

                            If (LOr (LEqual (OSYS, 0x07CE), LEqual (OSYS, 0x07CF)))
                            {
                                Store (0x2C, SMIF)
                                Store (0x00, TRP0)
                            }
                ...
                Device (COMB)
                {
                    ...
                    Method (_CRS, 0, NotSerialized)
                    {
                        CreateWordField (CRSA, \_SB.PCI0.LPCB.COMB._Y1D._MIN, MINA)
                        CreateWordField (CRSA, \_SB.PCI0.LPCB.COMB._Y1D._MAX, MAXA)
                        CreateWordField (CRSA, \_SB.PCI0.LPCB.COMB._Y1E._INT, INTA)
                        Store (Zero, MINA)
                        Store (Zero, MAXA)
                        Store (Zero, INTA)
                        CreateWordField (CRSB, \_SB.PCI0.LPCB.COMB._Y1F._MIN, MINB)
                        CreateWordField (CRSB, \_SB.PCI0.LPCB.COMB._Y1F._MAX, MAXB)
                        CreateWordField (CRSB, \_SB.PCI0.LPCB.COMB._Y20._INT, INTB)
                        CreateByteField (CRSB, \_SB.PCI0.LPCB.COMB._Y21._DMA, DMAB)
                        Store (Zero, MINB)
                        Store (Zero, MAXB)
                        Store (Zero, INTB)
                        Store (Zero, DMAB)
